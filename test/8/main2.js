...
*/
	...
	// ラベルの設定
	var SuperLabel = window.labels.SuperLabel;
...
*/
		...
		var SuperSprite = Class.create(Sprite, {
			// コンストラクタ
			initialize: function(x, y, lplayGame){
				//console.log('スーパークラスの継承');
				Sprite.call(this, x, y);
				this.playGame = lplayGame || playGame;
				return this;  
			},
			// 機体の追加処理
			addInstance: function(self) {
				//console.log('インスタンスの追加');
				this.playGame.addChild(self);
				return this;
			},
			// 機体の削除処理
			removeInstance: function(self) {
				//console.log('インスタンスの削除')
				this.playGame.removeChild(self);
				// 配列のdelete処理
				return this;
			},
			// ###############
