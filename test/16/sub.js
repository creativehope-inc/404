$(document).ready(function(){
	...
	var ZakoBase = Class.create( Enemy, {
		...
		_checkHP: function() {
			if ( this.hitpoint <= 0 ) {
				// 画面から敵要素を削除（見栄え的問題）
				console.log( !!(this.removeInstance( this ) ) );
	...
	var BossEnemy = Class.create( Enemy, {
					...
					if ( game.frame % 100 == 0 ) {
						...
						for ( var i = -4; i < 4; i ++ ) {
							for ( var j = -4; j < 4; j ++ ) {
								if ( i != 0 && j != 0 ) {
									// 発射処理
									new BossBullet2( self.x, self.y, pg, i, j );
								}
							}
						}
					}
		...
		checkHP: function( self, callback ) {
			if ( self.hitpoint <= 0 ) { // 0以下なら
				// 敵配列から該当要素を削除
				console.log( !!( delete enemyArr[ self.uuid ] ) );
				// 音を鳴らす
				if ( store.music ) self.bossCrashed.play();
				// コールバック実行
		...
		initialize: function( x, y, uuid, pg ) {
			...
			$(this).on( 'enterframe' , function() {
				...
				if ( !flag ) flag = self.checkHP( self , function() {
				   	...
				   	store.MajiFlag = true;
				   	// インスタンスの削除
				   	console.log( !!( self.removeInstance( self ) ) );
				});			
			});
		}
	});
